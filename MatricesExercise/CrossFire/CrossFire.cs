namespace CrossFire
{
    ...
    class CrossFire
    {
        ...
        private static bool IsInMatrix(int rowIndex, int colImpact, List<List<int>> matrix)
        {
            if (rowIndex >= 0 && rowIndex < matrix.Count && colImpact >= 0 && colImpact < matrix[rowIndex].Count)
            {
                return true;
            }
            return false;
